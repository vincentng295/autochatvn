    ...
    while True:
        ...
        try:
            ...
            for msg in reversed(driver.find_elements(By.CSS_SELECTOR, "div.message")):
                ...
                if bubble:
                    driver.execute_script("arguments[0].click();", bubble[0])
                    time.sleep(0.5)
                time_span = msg.find_elements(By.CSS_SELECTOR, "span.time")
                if time_span:
                ...
                if is_me:
                    if msg_list:
                        break
                    new_list.insert(0, {"role": "me", "message": text, "time": time_text})
                elif is_you:
                    new_list.insert(0, {"role": "stranger", "message": text, "time": time_text})
                else:
